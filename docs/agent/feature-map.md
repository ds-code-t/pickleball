...
| Pickleball Studio isolated application, workspace/file/process services, managed process lifecycle/history/output, self-contained Maven execution, and Streamable-HTTP MCP foundation | `pickleball-studio`; `pickleball-studio/src/main/java/tools/dscode/studio/workspace`; `pickleball-studio/src/main/java/tools/dscode/studio/process`; `pickleball-studio/src/main/java/tools/dscode/studio/build`; `pickleball-studio/src/main/java/tools/dscode/studio/mcp`; `src/main/java/tools/dscode/studio/launcher`; `gradle/pickleball-studio.gradle`; search `StudioApplication`, `WorkspaceFileService`, `WorkspaceProcessService`, `ManagedProcessService`, `ProcessOutputChunk`, `MavenToolchainService`, `MavenBuildService`, `StudioMcpTools`, `StudioMcpConfiguration`, `StudioServer`, `verifyBundledStudio` | `pickleball-studio/src/test/java`; `WorkspaceFileServiceTest`; `WorkspaceProcessServiceTest`; `ManagedProcessServiceTest`; `MavenToolchainServiceTest`; `MavenBuildServiceTest`; `StudioMcpToolsTest`; `StudioMcpContextTest`; `:pickleball-studio:test`; `:pickleball-studio:verifyBundledStudio` | The consumer dependency remains the single `tools.dscode:pickleball` artifact; Studio is a nested Spring Boot application running in a separate JVM; Spring/Spring AI dependencies and the opaque Studio-managed Maven runtime remain isolated from the consumer Maven dependency graph; managed process history is Studio-session scoped | `docs/pickleball-studio.md`; `pickleball-studio/AGENTS.md` |
