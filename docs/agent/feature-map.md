...
| Pickleball Studio isolated application, workspace file services, and Streamable-HTTP MCP foundation | `pickleball-studio`; `pickleball-studio/src/main/java/tools/dscode/studio/workspace`; `pickleball-studio/src/main/java/tools/dscode/studio/mcp`; `src/main/java/tools/dscode/studio/launcher`; `gradle/pickleball-studio.gradle`; search `StudioApplication`, `WorkspaceFileService`, `StudioMcpTools`, `StudioMcpConfiguration`, `StudioServer`, `verifyBundledStudio` | `pickleball-studio/src/test/java`; `WorkspaceFileServiceTest`; `StudioMcpToolsTest`; `StudioMcpContextTest`; `:pickleball-studio:test`; `:pickleball-studio:verifyBundledStudio` | The consumer dependency remains the single `tools.dscode:pickleball` artifact; Studio is a nested Spring Boot application running in a separate JVM; Spring/Spring AI dependencies remain inside the nested Studio JAR and are not consumer dependencies | `docs/pickleball-studio.md`; `pickleball-studio/AGENTS.md` |
