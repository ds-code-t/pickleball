package tools.dscode.modkit.blackbox;

import net.bytebuddy.asm.Advice;
import net.bytebuddy.implementation.bytecode.assign.Assigner;

public final class MethodAdvice {
    private MethodAdvice() {}

    @Advice.OnMethodEnter(skipOn = Advice.OnNonDefaultValue.class)
    static Object[] enter(
            @Advice.Origin("#t") String typeName,
            @Advice.Origin("#m") String methodName,
            @Advice.This(optional = true) Object self,
            @Advice.AllArguments(readOnly = false, typing = Assigner.Typing.DYNAMIC) Object[] args
    ) throws Throwable {
        final String key = Plans.keyFor(typeName, methodName, args == null ? 0 : args.length);
        final Plans.MethodPlan p = Registry.methodPlan(key);
        if (p == null) return null;
        System.out.println("[Advice][enter] Matched plan for " + key + (p.returnTypeFqcn != null ? (" returns " + p.returnTypeFqcn) : ""));

        if (p.bypass != null && p.bypass.bypass(args)) {
            Object value = (p.bypassReturn != null) ? p.bypassReturn.supply(args) : null;
            System.out.println("[Advice][enter] Bypassing original for " + key + ", returning=" + value);
            return new Object[]{ value }; // non-null => skip original
        }
        if (p.before != null) {
            System.out.println("[Advice][enter] Applying BEFORE mutator for " + key);
            p.before.mutate(args);
        }
        return null;
    }

    @Advice.OnMethodExit(onThrowable = Throwable.class)
    static void exit(
            @Advice.Origin("#t") String typeName,
            @Advice.Origin("#m") String methodName,
            @Advice.This(optional = true) Object self,
            @Advice.AllArguments Object[] args,
            @Advice.Return(readOnly = false, typing = Assigner.Typing.DYNAMIC) Object ret,
            @Advice.Thrown(readOnly = false) Throwable thrown,
            @Advice.Enter Object[] bypassCarrier
    ) throws Throwable {
        final String key = Plans.keyFor(typeName, methodName, args == null ? 0 : args.length);
        final Plans.MethodPlan p = Registry.methodPlan(key);
        if (p == null) return;

        if (bypassCarrier != null) { // skipped original
            ret = bypassCarrier.length > 0 ? bypassCarrier[0] : null;
            thrown = null;
            System.out.println("[Advice][exit] Original skipped for " + key + ", returning=" + ret);
            return;
        }
        if (thrown != null && p.onThrow != null) {
            System.out.println("[Advice][exit] Applying ON_THROW mutator for " + key + ": " + thrown);
            thrown = p.onThrow.mutate(args, ret, thrown);
        }
        if (p.afterSelf != null) {
            System.out.println("[Advice][exit] Applying AFTER_SELF mutator for " + key);
            ret = p.afterSelf.mutate(self, args, ret, thrown);
        } else if (p.after != null) {
            System.out.println("[Advice][exit] Applying AFTER mutator for " + key);
            ret = p.after.mutate(args, ret, thrown);
        } else {
            System.out.println("[Advice][exit] No AFTER mutator for " + key);
        }
    }
}
