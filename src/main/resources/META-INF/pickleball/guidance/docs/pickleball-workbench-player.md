...
The center editor is one Gherkin document shown as ordinary text or as the snap-editor Pickleball pack in JavaFX `WebView`. A prominent **Text | Blocks** toggle switches those views of the same buffer. Import/export is a canonical round-trip (structure-preserving; whitespace may normalize). Play, Step, and From Here keep using `LiveScenarioPlayer`. **Play** executes a derived plan: Background steps plus the selected scenario, with one Examples row substituted when selected. Blocks are Gherkin text, including `Given` / `When` / `Then`. Nested steps and `IF` / `ELSE` snap as parent/child using leading colons. Clicking a block or line instantly seeks the playhead. Hover, pickup, and snap highlights are cosmetic only; the live buffer is rewritten only when a block is dropped onto a new snap. The top player bar shows the Pickleball version. **Block actions** (default right-side tab, **View > Block action log**) records those editor-local events. JavaScript never executes Gherkin.

Workbench chose OpenJFX `WebView` + `JFXPanel` over JCEF so the browser panel stays a Workbench-only Maven dependency resolved onto the forked controller classpath. JDK 21 does not ship a modern browser component. If JavaFX cannot start, the same `LiveScenarioPlayer` buffer remains editable as plain Gherkin text and Blocks is shown as unavailable. The JavaFX unnamed-module warning on Windows is expected and is not a hang; the player window is shown before WebView starts.
