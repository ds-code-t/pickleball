...
When the evidence supports a bounded rerun, use the selected run index's `runProfile` as the authoritative starting RunVar set instead of reconstructing execution configuration from runner defaults, property files, named profiles, system properties, or Cucumber aliases. Use the compact `pkb_run_profile` form for simple values or expanded `pkb_run_profile.<pkb_var>` members when nested assignment parsing would make punctuation-heavy values awkward. Do not mix the two forms. Both resolve to the same direct RunVar model; change only the RunVars required by the current hypothesis.

`runProfileFingerprint` is the SHA-256 fingerprint of the canonical final execution RunVars. Actual protected values participate in the one-way hash, while `runProfile` contains only safe/protected representations. `directRunProfile=true` identifies a run launched from an explicit full RunVar override. Use `runProfileFingerprint` as the equality check for final RunVars; `configurationHash` is broader and may also reflect configuration provenance/representation. These fields let an agent verify the intended execution contract before comparing deeper evidence.
