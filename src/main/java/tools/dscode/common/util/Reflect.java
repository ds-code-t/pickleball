...
public final class Reflect {
    ...
    /**
     * Create a new instance via the best-matching declared constructor. Returns
     * null on failure.
     */
    public static Object newInstance(Class<?> clazz, Object... args) {
        ...
        for (Constructor<?> c : ctors) {
            if (acceptsArgCount(c, a.length))
                byCount.add(c);
        }
        if (byCount.isEmpty()) {
            // No exact/vararg count match; fall back to first and hope for the
            // best
            return tryConstruct(ctors[0], a);
        }
        if (byCount.size() == 1) {
    ...
    /**
     * Invoke a method (public or private), supporting both instance and static
     * targets.
     * <p>
     * Usage: - Instance: invokeAnyMethod(someObject, "doThing", args...) -
     * Static: invokeAnyMethod(SomeClass.class, "staticThing", args...)
     * <p>
     * Matching order: 1) by name (instance methods preferred when target is an
     * instance; static-only if target is Class), 2) then by argument count
     * (varargs accepted), 3) then by argument types (best assignable match).
    ...
    private static void makeAccessible(Method m, Object receiver) {
        try {
            ...
            // Best effort only; if JPMS blocks it, invoke will fail and we
            // return null.
        }
    }
    ...
    /**
     * Higher score = better match. Prefers non-varargs, exact type matches,
     * then assignable matches.
     */
    ...
    private static int matchScore(Class<?> paramType, Object arg) {
        if (arg == null)
            return paramType.isPrimitive() ? Integer.MIN_VALUE : 1;
    ...
    private static Object[] prepareArgsForVarargs(Method m, Object[] args) {
        ...
        int fixed = pt.length - 1;
        if (args.length == pt.length) {
            // Might already be passed as a single array for the vararg – let
            // reflection handle it as-is.
            Object last = args[args.length - 1];
            if (last != null && last.getClass().isArray()
                    && pt[pt.length - 1].isAssignableFrom(last.getClass())) {
                return args;
    ...
    public static Object getDirectProperty(Object target, String name) {
        ...
        if (bean == null)
            bean = findZeroArgMethod(target.getClass(), "is" + cap);
        if (bean != null) {
            try {
                if (!bean.canAccess(target))
    ...
    /**
     * Multi-hop: supports dot-separated paths like "propA.propB.propC". Each
     * segment is resolved via getDirectProperty(..).
     */
    public static Object getProperty(Object target, String name) {
        ...
        for (String raw : parts) {
            ...
            if (segment.isEmpty())
                continue; // tolerate accidental ".."
            current = getDirectProperty(current, segment);
            if (current == null)
                return null; // any miss -> null
        }
        return current;
    }
    ...
    /**
     * Multi-hop: supports dot paths like "a.b.c" (sets 'c' on the resolved
     * 'b').
     */
    ...
    private static Method findOneArgMethod(Class<?> type, String name, Object value) {
        for (Class<?> c = type; c != null; c = c.getSuperclass()) {
            for (Method m : c.getDeclaredMethods()) {
                ...
                if (value == null) {
                    if (!p.isPrimitive())
                        return m; // null ok for non-primitive
                } else {
                    Class<?> want = p.isPrimitive() ? wrapperType(p) : p;
                    if (want.isAssignableFrom(value.getClass()))
                        return m;
                }
            }
        }
