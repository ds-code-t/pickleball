package tools.dscode.modkit.extensions;

import tools.dscode.cucumberextended.annotations.NoLogging;
...
import tools.dscode.modkit.coredefinitions.MetaSteps;
import tools.dscode.modkit.executions.StepExecution;
import tools.dscode.modkit.mappings.ParsingMap;
import tools.dscode.modkit.status.SoftException;
import tools.dscode.modkit.status.SoftRuntimeException;
import tools.dscode.modkit.util.PickleStepArgUtils;
import tools.dscode.modkit.util.Reflect;
...
import static tools.dscode.modkit.contrib.cucumber.CucumberModKitBootstrap.META_FLAG;
import static tools.dscode.modkit.contrib.cucumber.CucumberModKitBootstrap.SKIP_LOGGING;
import static tools.dscode.modkit.coredefinitions.FlagSteps.*;
import static tools.dscode.modkit.coredefinitions.MappingSteps.DocString_KEY;
import static tools.dscode.modkit.coredefinitions.MappingSteps.TABLE_KEY;
import static tools.dscode.modkit.coredefinitions.MetaSteps.defaultMatchFlag;
import static tools.dscode.modkit.evaluations.AviatorUtil.eval;
import static tools.dscode.modkit.state.ScenarioState.getScenarioState;
import static tools.dscode.modkit.util.ArgumentUtility.emptyDataTable;
import static tools.dscode.modkit.util.ArgumentUtility.emptyDocString;
import static tools.dscode.modkit.util.ExecutionModes.RUN;
import static tools.dscode.modkit.util.ExecutionModes.SKIP;
import static tools.dscode.modkit.util.KeyFunctions.getUniqueKey;
import static tools.dscode.modkit.util.Reflect.getProperty;
import static tools.dscode.modkit.util.Reflect.invokeAnyMethod;
import static tools.dscode.modkit.util.stepbuilder.StepUtilities.createScenarioPickleStepTestStep;
import static tools.dscode.modkit.util.stepbuilder.StepUtilities.getDefinition;
...
//            this.overRideUUID = SKIP_LOGGING;
//        }
        ...
        if (isDocStringStep) {
            ...
            if (docStringName != null && !docStringName.isBlank())
                getScenarioState().getParsingMap().getRootSingletonMap().put("DOCSTRING."+docStringName.trim(), docString);
//            if (docStringName != null && !docStringName.isBlank())
//                getScenarioState().register(docString, docStringName.trim());
            ...
            if (tableName != null && !tableName.isBlank())
                getScenarioState().getParsingMap().getRootSingletonMap().put("DATATABLE."+tableName.trim(), dataTable);
//            if (tableName != null && !tableName.isBlank())
//                getScenarioState().register(dataTable, tableName.trim());
    ...
    public StepExtension buildNewStep(String newStepText) {
        ...
        PickleStep pickleStep = new PickleStep(newPickleStepArgument, rootStep.getAstNodeIds(), rootStep.getId(), rootStep.getType().orElse(null), newStepText);
