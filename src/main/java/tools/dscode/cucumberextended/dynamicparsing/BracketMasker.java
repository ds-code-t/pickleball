...
/**
 * Minimal bracket masker: - Extends HashMap: placeholder -> inner (unbracketed)
 * value - Masks deepest-first, then outward, with order: () -> {} -> [] ->
 * &lt;> - Placeholders look like "\u2408_1", "\u2408_2", ... (untypable prefix
 * ␈) - Restores by re-wrapping each placeholder with its original bracket type
 * Notes: - "Same-bookend characters are not allowed inside" is enforced by
 * patterns like: \\([^()]*\\), \\{[^{}]*\\}, \\[[^\\[\\]]*\\], &lt;[^&lt;>]*>
 * which match only non-nested segments of the SAME bracket type; we loop until
 * no change to achieve deepest-first masking.
 */
public final class BracketMasker extends HashMap<String, String> {
    private static final long serialVersionUID = 1L;

    // Untypable prefix used in all placeholder keys (U+2408 "SYMBOL FOR
    // BACKSPACE": ␈)
    ...
    public BracketMasker(String input) {
        ...
        for (;;) {
            ...
            // Process each bracket type to exhaustion (deepest-first for that
            // type)
            maskAllOfType('(', ')');
            maskAllOfType('{', '}');
            maskAllOfType('[', ']');
            ...
            if (masked.equals(before))
                break; // no changes in this entire pass
        }
    }
    ...
    public String restoreFrom(String textWithPlaceholders) {
        ...
        for (;;) {
            ...
            if (before.indexOf(KEY_PREFIX) < 0)
                break;
    ...
    /**
     * Mask all innermost occurrences of a single bracket type until none
     * remain.
     */
    private void maskAllOfType(char open, char close) {
        // Build a pattern matching innermost segments for this bracket type
        // only.
        // Examples:
        // '(': "\\([^()]*\\)"
        // '{': "\\{[^{}]*\\}"
    ...
    /**
     * Create a local counter that continues global numbering based on current
     * map size.
     */
    ...
    /**
     * Innermost pattern for a bracket pair, matching no same-bracket chars
     * inside.
     */
    private static String innermostPatternFor(char open, char close) {
        ...
        String negClass;
        if (open == '[') {
            // In a character class, ']' must be escaped specially; we avoid
            // char classes for '[' by explicit negation.
            // But for simplicity and clarity, we stick with a negated class
            // form and escape both.
            negClass = "[^\\[\\]]*";
        } else if (open == '(') {
            negClass = "[^()]*";
        } else if (open == '{') {
