...
public abstract class PickleballRunner {
    ...
    protected PickleballRunner() {
        ...
        /* Build default_profile from the fully resolved normal project configuration. */
        applyPkbAliases();
        ...
        debug("Final values after profile/RunVar resolution + alias sync: " + values);
    ...
    /** True when the current runner was resolved from direct {@code pkb_runvars}. */
    ...
    private void applyDebugFlags() {
        ...
        if (debugArgs != null && !debugArgs.isBlank()) {
            debugFlags.addAll(Arrays.stream(debugArgs.split(","))
                    .filter(s -> !s.isBlank())
    ...
    private void publishToSystemProperties() {
        ...
        for (Map.Entry<String, String> entry : values.entrySet()) {
            ...
            if (PKB_RUN_PROFILE.equals(normalizePkbKey(key))) {
                skipped++;
                continue;
            }
        ...
        debug("Published " + count + " value(s) to system properties (skipped "
                + skipped + " derived/foreign-engine key(s))");
    }
    ...
    private void mergeResourcePropertiesIfMissing(String resourceName) {
        try {
            ...
            if (cl == null) cl = PickleballRunner.class.getClassLoader();
    ...
    private void mergeAllSystemProperties() {
        ...
        for (String key : sys.stringPropertyNames()) {
            ...
            if (PKB_props.isRunVariableKey(normalized)) {
                systemRunVarOverrides.put(normalized, value);
            }
            count++;
        }
    ...
    private static CliOptionProjection projectCucumberCliArgs(String[] args) {
        ...
        if (args == null || args.length == 0) return projection;
        for (int i = 0; i < args.length; i++) {
            ...
            switch (arg) {
                ...
                case "--help", "-h", "--version", "-v", "--publish", "--dry-run", "-d",
                     ...
                     "--wip", "-w" -> { }
                default -> {
                    if (!arg.startsWith("-")) projection.features.add(arg);
                }
            }
        }
        return projection;
    ...
    private static String joinTagExpressions(List<String> expressions) {
        ...
        return expressions.stream().map(String::trim).filter(s -> !s.isBlank())
                .map(s -> "(" + s + ")")
    ...
    private static String joinNameFilters(List<String> names) {
        ...
        return names.stream().map(String::trim).filter(s -> !s.isBlank())
                .map(s -> "(?:" + s + ")")
    ...
    private static String formatCliArgs(String[] args) {
        ...
        return Arrays.stream(args).map(arg -> arg == null ? "" : arg)
                .map(PickleballRunner::quoteCliArg)
                .collect(java.util.stream.Collectors.joining(" "));
    }

    private static String formatCliArgs(List<String> args) {
        return args == null || args.isEmpty() ? "" : formatCliArgs(args.toArray(new String[0]));
    }
    ...
    private void refreshRunProfile() {
        ...
        values.put(PKB_RUN_PROFILE, PickleballProfiles.serializeRunProfile(values));
    }
    ...
    private String formatPkbOptions() {
        ...
        for (Map.Entry<String, String> entry : values.entrySet()) {
            String key = entry.getKey();
            String value = entry.getValue();
            if (key == null || value == null || key.equals(PKB_OPTIONS) || key.equals(PKB_RUN_PROFILE)
                    || key.startsWith(PickleballProfiles.INLINE_PROFILE_PREFIX)
                    || PKB_props.isRunMetadataKey(key) || !key.startsWith(PKB_PREFIX) || value.isBlank()) {
                continue;
            }
            if (!out.isEmpty()) out.append(", ");
            out.append(formatPkbOptionName(key)).append("=")
...
//        logTrace("[DynamicSuiteBase] " + message);
    ...
    private void refreshPkbOptions() {
        ...
        if (formatted != null && !formatted.isBlank()) values.put(PKB_OPTIONS, formatted);
    }
