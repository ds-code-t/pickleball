...
/**
 * Represents the TestStep message in <a
 * href=https://github.com/cucumber/messages>Cucumber's message protocol</a>
 * <p>
 * A `TestStep` is derived from either a `PickleStep` combined with a
 * `StepDefinition`, or from a `Hook`.
 * <p>
 * When derived from a PickleStep: * For `UNDEFINED` steps `stepDefinitionIds`
 * and `stepMatchArgumentsLists` will be empty. * For `AMBIGUOUS` steps, there
 * will be multiple entries in `stepDefinitionIds` and
 * `stepMatchArgumentsLists`. The first entry in the stepMatchArgumentsLists
 * holds the list of arguments for the first matching step definition, the
 * second entry for the second, etc
 */
// Generated code
...
public final class TestStep {
    ...
    public TestStep(
        ...
        this.stepDefinitionIds = stepDefinitionIds == null ? null
                : unmodifiableList(new ArrayList<>(stepDefinitionIds));
        this.stepMatchArgumentsLists = stepMatchArgumentsLists == null ? null
                : unmodifiableList(new ArrayList<>(stepMatchArgumentsLists));
    }
    ...
    /**
     * Pointer to all the matching `StepDefinition`s (if derived from a
     * `PickleStep`).
     * <p>
