...
/**
 * Represents the Pickle message in <a
 * href=https://github.com/cucumber/messages>Cucumber's message protocol</a>
 * <p>
 * A `Pickle` represents a template for a `TestCase`. It is typically derived
 * from another format, such as
 * [GherkinDocument](#io.cucumber.messages.GherkinDocument). In the future a
 * `Pickle` may be derived from other formats such as Markdown or Excel files.
 * <p>
 * By making `Pickle` the main data structure Cucumber uses for execution, the
 * implementation of Cucumber itself becomes simpler, as it doesn't have to deal
 * with the complex structure of a
 * [GherkinDocument](#io.cucumber.messages.GherkinDocument).
 * <p>
 * Each `PickleStep` of a `Pickle` is matched with a `StepDefinition` to create
 * a `TestCase`
 */
// Generated code
...
public final class Pickle {
    ...
    public Pickle(
        ...
        this.astNodeIds = unmodifiableList(
            new ArrayList<>(requireNonNull(astNodeIds, "Pickle.astNodeIds cannot be null")));
    }
    ...
    /**
     * Points to the AST node locations of the pickle. The last one represents
     * the unique id of the pickle. A pickle constructed from `Examples` will
     * have the first id originating from the `Scenario` AST node, and the
     * second from the `TableRow` AST node.
     */
