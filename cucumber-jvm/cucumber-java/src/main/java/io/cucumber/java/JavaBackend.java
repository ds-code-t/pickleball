...
final class JavaBackend implements Backend {
    ...
    public void loadGlue(Glue glue, List<URI> gluePaths) {
        // PickleballChange
        List<URI> newGluePaths = Stream.concat(
            gluePaths.stream()
                    .filter(u -> !u.toString().startsWith("classpath:/tools/dscode/coredefinitions")),
            Stream.of(URI.create("classpath:/tools/dscode/coredefinitions"))).toList(); // creates
                                                                                        // a
                                                                                        // new
                                                                                        // immutable
                                                                                        // List
        System.out.println("@@gluePaths: " + gluePaths);
        System.out.println("@@newGluePaths: " + newGluePaths);
        newGluePaths.forEach(a -> System.out.println(a));
        ...
        newGluePaths.stream()
                ...
                .distinct()
                .forEach(aGlueClass -> {
                    if (!aGlueClass.isAssignableFrom(IgnoredSteps.class))
