...
public class JsonPathUtil {
    ...
    /**
     * A handle to a resolved node plus the info needed to replace it in its
     * parent.
     */
    public static final class NodeHandle {
        ...
        /**
         * The direct parent container: either ObjectNode or ArrayNode (may be
         * null if unresolved).
         */
        public final JsonNode parent;
        /**
         * The key under which this child lives, if parent is an ObjectNode
         * (else null).
         */
        public final String fieldName;
        /**
         * The index under which this child lives, if parent is an ArrayNode
         * (else null).
         */
    ...
    /**
     * Retrieve or create the final node addressed by a simple JSONPath-like
     * query. - Supports: "$.a.b[2].c", "settings.options[2]", "logs[]" -
     * Disallows wildcards/filters/recursion. Behavior: - If a segment is
     * "field[]" and it's the last segment: * Ensures an ArrayNode at that field
     * if create=true; returns a handle to that array (parent=obj,
     * fieldName="field"). - If a segment is "field[index]": * Pads the array
     * with nulls when create=true; returns a handle to that element if last, or
     * ensures the element is an ObjectNode to descend further when not last. -
     * If a segment is a plain field: * Creates an ObjectNode at that field when
     * needed and create=true (for descent or final node). Failures (or
     * create=false where something is missing) return a NodeHandle with all
     * fields null.
     */
    public static NodeHandle getOrCreate(ObjectNode root, String path, boolean create) {
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            if (br >= 0) {
                ...
                if ((maybeArr == null || maybeArr.isNull())) {
                    if (!create) {
                        System.out.println("[GOC] MISSING (create=false)");
                        return new NodeHandle(null, null, null, null);
                    }
                ...
                if (idxStr.isEmpty()) {
                    if (!last) {
                        System.out.println("[GOC] FAIL: cannot descend past []");
                        return new NodeHandle(null, null, null, null);
                    }
                ...
                while (arr.size() <= idx) {
                    if (!create) {
                        System.out.println("[GOC] MISSING index (create=false)");
                        return new NodeHandle(null, null, null, null);
                    }
                ...
                if (last)
                    return NodeHandle.ofArray(elem, arr, idx);

                if (elem == null || elem.isNull()) {
                    if (!create) {
                        System.out.println("[GOC] MISSING elem (create=false)");
                        return new NodeHandle(null, null, null, null);
                    }
                    elem = F.objectNode();
                    arr.set(idx, elem);
                ...
                if (!elem.isObject()) {
                    System.out.println("[GOC] FAIL: elem not object");
                    return new NodeHandle(null, null, null, null);
                }
                current = elem;
            } else {
                ...
                if (next == null || next.isNull()) {
                    if (!create) {
                        System.out.println("[GOC] MISSING next (create=false)");
                        return new NodeHandle(null, null, null, null);
                    }
                    next = F.objectNode();
                    obj.set(part, next);
                ...
                if (!next.isObject()) {
                    System.out.println("[GOC] FAIL: next not object");
                    return new NodeHandle(null, null, null, null);
                }
                current = next;
            }
