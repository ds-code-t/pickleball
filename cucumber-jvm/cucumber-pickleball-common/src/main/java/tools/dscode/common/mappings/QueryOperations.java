...
public class QueryOperations {
    ...
    // String topLevelReplacement = !topLevel.contains("[") ? topLevel + "[-1]"
    // : topLevel;
    // if (root != null) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        java.util.function.Function<com.fasterxml.jackson.databind.JsonNode, String> pp = n -> {
            try {
        ...
        if (ok2)
            passed++;
        pf.accept("[2] a.b.d overwrite", ok2);
        ...
        if (ok3)
            passed++;
        pf.accept("[3] a.'key with space' overwrite", ok3);
        ...
        boolean ok5 = r5
                && assertPtrEquals.apply("/data", "[{\"child\":{\"name\":\"ZZZ\"}},{\"child\":{\"name\":\"ZZZ\"}}]");
        if (ok5)
            passed++;
        pf.accept("[5] data[].child.name", ok5);
        ...
        boolean ok6 = r6
                && assertPtrEquals.apply("/people", "[{\"age\":17,\"nick\":\"Teen\"},{\"age\":18,\"nick\":\"Adult\"}]");
        if (ok6)
            passed++;
        pf.accept("[6] people[age >= 18].nick", ok6);
        ...
        if (ok7)
            passed++;
        pf.accept("[7] $.top array semantics", ok7);
        ...
        if (ok9)
            passed++;
        pf.accept("[9] nested quoted keys", ok9);
