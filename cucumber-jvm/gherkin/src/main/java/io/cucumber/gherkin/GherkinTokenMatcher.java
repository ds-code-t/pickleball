...
final class GherkinTokenMatcher implements TokenMatcher {
    ...
    @Override
    public void reset() {
        // TODO performance: reset() is called once in the constructor and once
        // for each file (Parser.parse()). It could be called only once, but
        // there is no measurable impact with the profiler
    ...
    private void setTokenMatched(
            Token token, TokenType matchedType, String text, String keyword, int indent, StepKeywordType keywordType,
            List<LineSpan> items
    ) {
