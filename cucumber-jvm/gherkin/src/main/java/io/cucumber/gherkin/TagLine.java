...
final class TagLine {
    ...
    static List<LineSpan> parse(int indent, String text, Location location) {
        ...
        while (indexEndOfLine < textLength && !(text.charAt(indexEndOfLine) == COMMENT_PREFIX_CHAR
                && isWhitespace(text.charAt(indexEndOfLine - 1)))) {
            indexEndOfLine++;
        }
        ...
        while (indexStartCurrentTag < indexEndOfLine) {
            ...
            // look for the end of current tag (going back from begin of next
            // tag)
