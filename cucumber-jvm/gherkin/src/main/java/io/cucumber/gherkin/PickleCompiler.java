...
final class PickleCompiler {
    ...
    private void compileRule(
            List<Pickle> pickles, Rule rule, List<Tag> parentTags, List<Step> featureBackgroundSteps, String language,
            String uri
    ) {
    ...
    private void compileScenario(
            List<Pickle> pickles, Scenario scenario, List<Tag> parentTags, List<Step> backgroundSteps, String language,
            String uri
    ) {
    ...
    private List<PickleStep> compilePickleSteps(
            List<Step> backgroundSteps, List<Step> scenarioSteps, List<TableCell> variableCells, TableRow valuesRow
    ) {
        if (scenarioSteps.isEmpty()) {
            // usually, a scenario has at least one step, but that's not
            // mandatory
            return emptyList();
        }
    ...
    private void compileScenarioOutline(
            List<Pickle> pickles, Scenario scenario, List<Tag> parentTags, List<Step> backgroundSteps, String language,
            String uri
    ) {
        List<Tag> scenarioTags = compileTags(parentTags, scenario.getTags());
        for (final Examples examples : scenario.getExamples()) {
            ...
            for (final TableRow valuesRow : examples.getTableBody()) {
                List<PickleStep> steps = compilePickleSteps(backgroundSteps, scenario.getSteps(), variableCells,
                    valuesRow);
                ...
                if (examples.scenarioTagsIndex != -1) {
                    TableCell cell = valuesRow.getCells().get(examples.scenarioTagsIndex);
                    tags.addAll(Arrays.stream(cell.getValue().trim().split("\\s+"))
                            .map(s -> new Tag(cell.getLocation(), s, "ST" + valuesRow.getId())).toList());
                }
                if (examples.componentTagsIndex != -1) {
                    TableCell cell = valuesRow.getCells().get(examples.componentTagsIndex);
                    tags.addAll(Arrays.stream(cell.getValue().trim().split("\\s+"))
                            .map(s -> new Tag(cell.getLocation(), COMPONENT_TAG_PREFIX + s, "CT" + valuesRow.getId()))
                            .toList());
                }
    ...
    @SuppressWarnings("ForLoopReplaceableByForEach") // classic 'for' loop is
    // ~2x faster than
    // 'for-each'
    private PickleTable pickleDataTable(
            DataTable dataTable, List<TableCell> variableCells, List<TableCell> valueCells
    ) {
    ...
    private PickleDocString pickleDocString(
            DocString docString, List<TableCell> variableCells, List<TableCell> valueCells
    ) {
        return new PickleDocString(
            docString.getMediaType().isPresent()
                    ? interpolate(docString.getMediaType().get(), variableCells, valueCells)
                    : null,
            interpolate(docString.getContent(), variableCells, valueCells));
    }

    private PickleStep pickleStep(
            Step step, List<TableCell> variableCells, TableRow valuesRow, StepKeywordType keywordType
    ) {
