...
public final class GherkinDialect {
    private static final Comparator<String> LONGEST_TO_SHORTEST_COMPARATOR = (a, b) -> Integer.compare(b.length(),
        a.length());
    ...
    GherkinDialect(
        ...
        this.stepKeywordsTypes = aggregateKeywordTypes(givenKeywords, whenKeywords, thenKeywords, andKeywords,
            butKeywords);
    }

    @SafeVarargs
    private static List<String> distinctKeywords(List<String>... keywords) {
        // french is the largest dialect with 32 keywords, so we build the
        // sorting hashset with this max size
    ...
    private static void addStepKeywordsTypes(
            Map<String, Set<StepKeywordType>> accumulator, StepKeywordType type, List<String> keywords
    ) {
     ...
     * @deprecated         use {{@link #getStepKeywordTypesSet(String)}}
     *                     instead.
     */
    @Deprecated
